...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const [fcResult, osmResult] = await Promise.all([
      firecrawlSearch(searchQuery),
      searchOverpass(county, state, providerType),
    ]);

    const debugErrors: string[] = [];
    if (fcResult.error) debugErrors.push(`Firecrawl: ${fcResult.error}`);
    if (osmResult.error) debugErrors.push(`Overpass: ${osmResult.error}`);

    let merged = mergeResults(fcResult.results, osmResult.results);
